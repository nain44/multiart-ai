...
<div class="tabs">
  <?php foreach (['pending' => 'Pending', 'approved' => 'Approved', 'rejected' => 'Rejected'] as $val => $label): ?>
    <a class="<?= $status === $val ? 'active' : '' ?>" href="/wallpapers/ai-queue?status=<?= $val ?>"><?= $label ?></a>
  <?php endforeach; ?>
</div>

<?php if ($status === 'rejected' && !empty($wallpapers)): ?>
  <form method="post" action="/wallpapers/ai-queue/clear-rejected"
      ...
      <div class="card ai-card">
        ...
        <div class="actions">
          <?php if ($status === 'pending'): ?>
            <form method="post" action="/wallpapers/ai-queue/<?= View::e($wp['_id']) ?>/approve">
            ...
            <form method="post" action="/wallpapers/ai-queue/<?= View::e($wp['_id']) ?>/reject">
              <input type="hidden" name="status" value="<?= $status ?>">
          ...
          <?php else: ?>
            ...
            <form method="post" action="/wallpapers/ai-queue/<?= View::e($wp['_id']) ?>/delete"
